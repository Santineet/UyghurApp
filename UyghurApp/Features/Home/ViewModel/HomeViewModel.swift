...
class HomeViewModel: NSObject {
    ...
    let audiosBR = BehaviorRelay<[AudiosModel]>(value: [])
    ...
    func getAudios(completion: @escaping (Error?) -> ()){
        if Network.instance.isConnected() == true {
            ServiceManager.instance.getAudios { (audiosArray, error) in
                if let error = error {
                    self.errorAudiosBR.accept(error)
                } else {
                    guard let audios = audiosArray as? [AudiosModel] else { return }
    ...
    func getVideos(completion: @escaping (Error?) -> ()){
        if Network.instance.isConnected() == true {
            ServiceManager.instance.getVideos { (videosArray, error) in
    ...
    func getDicCategories(completion: @escaping (Error?) -> ()){
        if Network.instance.isConnected() == true {
            ServiceManager.instance.getDicCategories { (categoriesArray, error) in
    ...
    func getStoreProducts(completion: @escaping (Error?) -> ()){
         if Network.instance.isConnected() == true {
            ServiceManager.instance.getStoreProducts { (productsArray, error) in
